...
export async function fetchClipInfo(
  ...
  const response = await fetch(url, {
    ...
    throw createAppError(500, "FetchClipInfoFailOnClient");
  });

  if (!response.ok) {
    throw createAppError(500, "FetchClipInfoFailOnServer");
  }
  ...
  await new HTMLRewriter()
    ...
    .text()
    .catch(() => {
      throw createAppError(500, "FetchClipInfoCannotParse");
    });
  ...
  try {
    ...
    const imageResponse = await fetch(imageUrl).catch(() => {
      throw createAppError(500, "FetchClipImageFailOnClient");
    });
    if (!imageResponse.ok) {
      throw createAppError(500, "FetchClipImageFailOnServer");
    }
    const imageArrayBuffer = await imageResponse.arrayBuffer().catch(() => {
      throw createAppError(500, "FetchClipImageNotArrayBuffer");
    });
